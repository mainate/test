...
bool objWindow::isRunningThread()
{
    return runThread;
}
...
void objWindow::startThread()
{
    // Start the thread
    int err = -1;
    if (functIndiport != 0) err = pthread_create(&threadsWin, NULL, functIndiport, (void*) &mutexNcurses);
    if (err == 0)
        runThread = true;
        else runThread = false;
}

void objWindow::stopThread()
{
    runThread = false;
    pthread_join(threadsWin, NULL);
    threadsWin = 0;
}

void objWindow::cancelThread()
{
    runThread = false;
    pthread_cancel(threadsWin);
    threadsWin = 0;
}
...
***********************************************************************************/
/*void maintextWindow::startThread()
{
    int err = -1;
    if (functIndiport != 0) err = pthread_create(&threadsWin, NULL, functIndiport, (void*) &mutexNcurses);
    if (err == 0)
...
void maintextWindow::stopThread()
{
    runThread = false;
   // pthread_cancel(threadsWin);
    pthread_join(threadsWin, NULL);
